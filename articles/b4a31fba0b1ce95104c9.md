---
title: "Deno を使って Vim/Neovim のプラグインを書く"
emoji: "🐜"
...
次に Deno の起動時型チェックを有効にします。以下を `.vimrc` に追記してください。
...
:::message
型チェックが有効な状態では、Deno の起動に時間がかかります。開発が落ち着いたら型チェックを無効化してください。
:::
...
touch denops/helloworld/mod.ts
```
...
Denops は自動的に `runtimepath` 内の `denops/*/mod.ts` を読み込みます。
そのため上記のような構造が Denops プラグインの基本型となります。

## 2. 骨組みの追加

Denops プラグインは [denops-std][] という Deno モジュールの [`start()`](https://doc.deno.land/https/deno.land/x/denops_std/mod.ts#start) 関数で初期化を行います。そのため `mod.ts` の内容を以下のように書き換えてください。
...
Denops では各プラグインが API を関数として登録します。まず、与えられた文字列を返却する `echo()` 関数を API として登録してみましょう。`mod.ts` を以下のように書き直してください。
...
先ほどの `echo` API を Vim のコマンドとして登録してみるので、以下のように `mod.ts` を変更してください。
...
迷路生成アルゴリズムから自作しても良いのですが、せっかく Deno を利用しているのでサードの迷路生成ライブラリである [maze_generator](https://deno.land/x/maze_generator@v0.4.0) を使います。
...
上記では `vim.cmd()` で Vim の `enew` コマンドを呼び出し新規バッファを現在の Window で開いた後 `vim.call()` で `setline()` 関数を呼ぶことでバッファに迷路を書き込んでいます。
これを実行すると以下のようになります。
