...
Denops プラグインは [denops-std][] という Deno モジュールの [`main()`](https://doc.deno.land/https/deno.land/x/denops_std/mod.ts#main) 関数で初期化を行います。そのため `mod.ts` の内容を以下のように書き換えてください。
