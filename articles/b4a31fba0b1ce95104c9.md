...
:::message
対象バージョン：[denops_std v1.0.0-beta.0](https://github.com/vim-denops/deno-denops-std/releases/tag/v1.0.0-beta.0) (2021.07.05)
:::
...
利用する場合は明示的に `denops#server#start()` や `denops#plugin#discover()` を呼び出す必要があります。
:::
...
:::message
Vim の場合、デバッグモードにしない限り Vim で発生したエラーを検出できません。
これは Vim の仕様なので、開発時は必ずデバッグモードを有効にしてください。
Neovim の場合は、非デバッグモードでも Neovim で発生したエラーを検出可能です。
:::
...
:::message
Vim を再起動するのが面倒な方は `:call denops#server#restart()` として Denops を再起動するか
`:call denops#plugin#register({プラグイン名}, {main.ts へのパス})` としてプラグインを再登録するとリロードできます。
:::
